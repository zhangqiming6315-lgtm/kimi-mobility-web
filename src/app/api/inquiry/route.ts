...
const distributorLimits = {
  ...
  productInterest: 30,
  quantity: 30,
  brandingRequirement: 30,
  message: 5000,
} as const;
const distributorBusinessTypes = new Set(["Importer", "Distributor", "Dealer Group", "Fleet & Distribution", "Other"]);
const distributorProductInterests = new Set(["Delivery X1", "Urban X1", "Utility X1", "Multiple Platforms", "Not Sure Yet"]);
const distributorQuantities = new Set(["Sample / Evaluation", "10–50", "51–100", "100+", "Not Sure Yet"]);
const distributorBrandingRequirements = new Set(["KIMI Brand", "Private Label", "To Be Discussed"]);
...
async function handleDistributorInquiry(input: Record<string, unknown>) {
  type DistributorInquiry = Record<keyof typeof distributorLimits, string>;
  const inquiry = Object.fromEntries(
    Object.keys(distributorLimits).map((key) => [
      key,
      typeof input[key] === "string" ? input[key].trim() : "",
    ]),
  ) as DistributorInquiry;
  const requiredFields: (keyof DistributorInquiry)[] = ["name", "company", "email", "market", "businessType", "productInterest", "quantity", "message"];
  const invalidFields = (Object.keys(distributorLimits) as (keyof DistributorInquiry)[]).filter(
    (key) => inquiry[key].length > distributorLimits[key],
  );
  for (const key of requiredFields) if (!inquiry[key]) invalidFields.push(key);
  if (!emailPattern.test(inquiry.email)) invalidFields.push("email");
  if (!distributorBusinessTypes.has(inquiry.businessType)) invalidFields.push("businessType");
  if (!distributorProductInterests.has(inquiry.productInterest)) invalidFields.push("productInterest");
  if (!distributorQuantities.has(inquiry.quantity)) invalidFields.push("quantity");
  if (inquiry.brandingRequirement && !distributorBrandingRequirements.has(inquiry.brandingRequirement)) invalidFields.push("brandingRequirement");

  const uniqueInvalidFields = [...new Set(invalidFields)];
  if (uniqueInvalidFields.length) {
    return NextResponse.json(
      { ok: false, code: "VALIDATION_ERROR", message: "Please check the required fields and try again.", fields: uniqueInvalidFields },
      { status: 422 },
    );
  }
  ...
  const rows: [string, string][] = [
    ...
    ["Expected Quantity", inquiry.quantity],
    ["Branding Requirement", inquiry.brandingRequirement || "Not specified"],
  ];
  const subject = ["New KIMI Distributor RFQ", inquiry.market, inquiry.company]
    .map(singleLine)
    .filter(Boolean)
    .join(" — ");
  const text = [
  ...
  const html = `<!doctype html><html lang="en"><body style="margin:0;background:#f2f2f0;color:#111111;font-family:Arial,Helvetica,sans-serif;"><div style="display:none;max-height:0;overflow:hidden;opacity:0;">New Distributor RFQ from ${escapeHtml(inquiry.name)}.</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f2f2f0;"><tr><td align="center" style="padding:32px 16px;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:680px;background:#ffffff;border-collapse:collapse;"><tr><td style="background:#111111;padding:28px 32px;"><img src="https://kimimobility.com/email/kimi-email-logo.png" alt="KIMI Mobility" width="150" style="display:block;width:150px;height:auto;border:0;"><p style="margin:24px 0 0;color:#00b894;font-size:12px;font-weight:700;letter-spacing:2.4px;line-height:18px;">NEW DISTRIBUTOR RFQ</p></td></tr><tr><td style="padding:32px;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">${rows.map(([label, value]) => `<tr><td valign="top" style="width:160px;border-bottom:1px solid #e6e6e6;padding:14px 16px 14px 0;color:#8a8a8a;font-size:12px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;">${escapeHtml(label)}</td><td valign="top" style="border-bottom:1px solid #e6e6e6;padding:14px 0;color:#111111;font-size:15px;line-height:23px;">${escapeHtml(value)}</td></tr>`).join("")}</table><div style="margin-top:28px;border-left:3px solid #00b894;padding:4px 0 4px 20px;"><p style="margin:0 0 10px;color:#8a8a8a;font-size:12px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;">Project Requirements</p><p style="margin:0;color:#111111;font-size:15px;line-height:24px;">${escapeHtml(inquiry.message).replace(/\n/g, "<br>")}</p></div></td></tr><tr><td style="background:#111111;padding:26px 32px;color:#ffffff;"><p style="margin:0;font-size:15px;font-weight:700;line-height:22px;">KIMI Mobility</p><p style="margin:6px 0 0;color:#a7a7a7;font-size:12px;line-height:19px;">Shanghai Kaimai New Energy Technology Co., Ltd.</p><p style="margin:12px 0 0;font-size:12px;line-height:19px;"><a href="https://kimimobility.com" style="color:#00b894;text-decoration:none;">kimimobility.com</a></p></td></tr></table></td></tr></table></body></html>`;
...
export async function POST(request: Request) {
  ...
  if (input.formType === "distributor_rfq") return handleDistributorInquiry(input);
